...
pub const RIJNDAEL_AES_SBOX: [u8; 256] = [
...
pub fn invers_sbox(sbox: [u8; 256]) -> Result<[u8; 256], String> {
    if sbox.is_empty() {
        return Err(String::from("Sbox is empty"));
    }
    let mut result: [u8; 256] = [0; 256];
    for i in 0..sbox.len() {
        result[i] = sbox.iter().position(|&x| x as usize == i).unwrap() as u8;
    }
    Ok(result)
}