...
pub fn generate_pbox(key: String, size: usize) -> Result<Vec<u8>, String> {
    if key.is_empty() {
...
pub fn encrypt_pbox(input_str: String, p_box: Vec<u8>) -> Result<Vec<u8>, String> {
    ...
    for i in 0..p_box.len() {
        let temp = str_bytes[p_box[i] as usize] as u8;
        result.push(temp);
    }
    Ok(result)
}

pub fn decrypt_pbox(input: Vec<u8>, p_box: Vec<u8>) -> Result<String, String> {
    if input.is_empty() {
...
pub fn invers_pbox(input: Vec<u8>) -> Result<Vec<u8>, String> {
    if input.is_empty() {
        return Err(String::from("P-box is empty"));
    }
    let mut result: Vec<u8> = vec![];
    ...
    for (index, value) in input.iter().enumerate() {
        result[*value as usize] = index as u8;
    }
    Ok(result)
}